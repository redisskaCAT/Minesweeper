...
ConsoleDisplay::ConsoleDisplay(Field* f, Coord _sizeField, int mines) {
  ...
  sizeField.x = _sizeField.x * (shiftAlf + 1) + shiftNum + 1;
  sizeField.y = _sizeField.y + 1;

  sizeScreen.x = sizeField.x + margin.x * 2;
  sizeScreen.y = sizeField.y + 4 + 4 * margin.y;
  ...
  cEnter0 = Coord(cScreen.x + margin.x, cScreen.y + sizeScreen.y - 4);
  cEnter.y = cEnter0.y;
}
...
void ConsoleDisplay::showVal(Coord coord, Value val, bool active) {
  ...
  CUP(Coord(cField.x + (shiftAlf + 1) * coord.x + (shiftNum + 1),
            cField.y + coord.y + 1));
...
void ConsoleDisplay::showEnter(int i) {
  string strEnter[2] = {" Enter the cell : ", " ERROR! Repeat enter : "};

  cout << "\033[37;40m";
  CUP(cEnter0);
  cout << strEnter[i];
  for (int j = 0; j < sizeField.x - strEnter[i].size(); j++) {
    cout << ' ';
  }

  cEnter.x = cEnter0.x + strEnter[i].size();

  CUP(cEnter);
}

void ConsoleDisplay::showTimer(int time) {
  CUP(cTimer);
  cout.width(3);
  cout.fill('0');
  cout << time << endl;
...
void ConsoleDisplay::showField() {
  for (int i = -1; i < rowsField; i++) {
    CUP(Coord(cField.x, cField.y + i + 1));
    for (int j = -1; j < colsField; j++) {
      if (i < 0 || j < 0)
        showFrame(Coord(j, i));
      else
      {
...
void ConsoleDisplay::showEndGameMessange(bool win) {
  ...
  if (win) {
    CUP(Coord(center.x - 3, cEnter0.y));
    cout << "\033[32m"
         << "YOU WIN!"
         << "\033[39m";
  } else {
    CUP(Coord(center.x - 4, cEnter0.y));
    cout << "\033[31m"
         << "YOU LOSE!"
         << "\033[39m";
  }
